...
Neste trabalho foi criada uma camada \textit{Network} comum para todos os nós e uma camada \textit{Application} por cada tipo de nó que implementa cada um dos tipos descritos. Nas próximas secções é feita uma exposição detalhada de todo o sistema.
...
O sistema \textit{HORUS} é um método probabilístico. Neste trabalho optou-se por parametrizar o sinal recebido o que permite obter uma distribuição normal que se ajusta ao histograma das potências recebidas, tal como podemos ver na Figura \ref{fig:23:horusNormal}. Esta abordagem garante uma continuidade dos valores obtidos.
...
Na fase \textit{online} é necessário calcular a probabilidade de um nó estar numa determinada posição. Para um vector de potências médias de nós estáticos recebidas no nó móvel dado por  \begin{math}s=(s_1,...,s_k)\end{math}, pretende-se encontrar a posição \begin{math}x\end{math} que maximiza a probabilidade \begin{math}P(x/s)\end{math}. Para obter esta probabilidade usando a função densidade de probabilidade registada no \textit{RadioMap} recorremos à equação:
...
onde \begin{math}fdp(q)\end{math} é a função densidade de probabilidade normal determinada na posição \begin{math}x\end{math} dada por \ref{eq1}.

Dado que queremos evitar a utilização de algoritmos numéricos, computacionalmente pesados, para a resolução do integral, transforma-se a função densidade de probabilidade encontrada para cada nó estático, numa função de distribuição normal standard, cujos valores podem ser tabelados. Para efeitos de simulação criou-se por isso, um ficheiro XML que contém os valores necessários e que é carregado no nó base onde é determinada a posição.
...
A posição do \textit{RadioMap} escolhida para é a que tem probabilidade dada por:

\begin{equation}
argmax_x[P(s/x)]
\end{equation}

Obtida a posição usando o \textit{Discrete-Space Estimator} torna-se necessário usar o \textit{Continous-Space Estimator} para chegar a um valor mais próximo da localização real, uma vez que no estimador discreto são apenas consideradas posições medidas durante a fase \textit{offline}.
