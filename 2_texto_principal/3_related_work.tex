...
Designam-se \acfp{CM} aos profissionais de saúde que prestam cuidados ao domicílio (ex:enfermeiros,médicos). Os \acs{CM}s interagem de forma activa com as pessoas idosas presencialmente ou por telefone. Avaliam a habilidade do idoso e a sua predisposição para a introdução de novos equipamentos. Uma parte significativa da monitorização do \acs{CM} são as chamadas \acfp{ADL}, uma lista de actividades que permite medir a função cognitiva e física do idoso (Tabela \ref{tab:1:adls}).
...
A partir da Figura \ref{fig:3:localizationClass} é possível fazer algumas observações relativas ao tema deste trabalho que permitirá reduzir a bibliografia a consultar.

\textbf{Área de Instalação}: A área de instalação deverá ser local dado uma vez que estamos perante um ambiente doméstico.

\textbf{PHY}: O meio de transmissão do sinal deverá ser a \acs{RF} (Rádio-Frequência) 
