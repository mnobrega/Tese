...
É então sugerido um termo para este tipo de perdas:

\begin{equation}
	L_{obs}[dB] = \beta{n} + \gamma{d_m}
\end{equation}

onde \begin{math}n\end{math} é o número de vezes que a fronteira do obstáculo é atravessado por um segmento de recta entre o emissor e o receptor, \begin{math}d_m\end{math} é a profundidade (em metros) do obstáculo e \begin{math}\beta{}\end{math} e \begin{math}\gamma{}\end{math} são constantes obtidas experimentalmente que representam a atenuação por parede e a atenuação por metro respectivamente. Para o trabalho \cite{35} os valores obtidos foram \begin{math}\beta{}\approx{9dB}\end{math} e \begin{math}\gamma{}\approx{0.4dB/m}\end{math}. 

Devido à impossibilidade de utilizar nós reais neste trabalho, por condicionantes relacionadas com o custo envolvido, que permitissem chegar a valores realistas, optou-se por considerar, com base no Tabela \ref{tab:1:attenuationPerInch} do manual da \textit{3Com Wireless Antenas}\footnote{http://www.scribd.com/doc/32613170/3Com\%C2\%AE-Wireless-Antennas}, os seguintes valores:
...
\begin{tabular}{ |c|c|c|}
	\hline
  	...
  	\hline
\end{tabular}
	\caption{Valores de atenuação por parede e por metro usados neste trabalho.}
